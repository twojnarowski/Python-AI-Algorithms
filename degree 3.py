...
def fitness(points, poly):
    ...
    for point in points:
        if point[2]==0:
            if point[1] > poly[0] * point[0] ** 3 + poly[1] * point[0]**2 + poly[2]*point[0]+poly[3]:
                score1 += 1
            total += 1
        elif point[2]==1:
            if  point[1] < poly[0] * point[0] ** 3 + poly[1] * point[0]**2 + poly[2]*point[0]+poly[3]:
...
def computePerfPopulation(population, points):
    ...
    for individual in population:
        populationperf[individual[4]]=  (fitness(points, individual))
...
def mutatePoly(poly):
    poly[int(random.random() * 4)] = randrange(1,10)
    return poly
...
# Data for plotting
t = np.arange(0, 10, 1)
s = result[0]*(t**3) + result[1]*(t**2) + result[2]*t+result[3]
